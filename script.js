    ...
    let currentKey = ''; // Добавлено для отслеживания текущего ключа
    ...
    document.querySelectorAll('.detail-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            // Сохраняем ключ текущего показателя
            currentKey = key;
    ...
    function updateDetailView() {
        ...
        const isPremiumMortgage = currentKey.includes('Льготная ипотека') && 
                                 currentKey.includes('Количество выданных кредитов');
    ...
    function createTable(data, container, sectionTitle, formatAsPeriod, isPremiumMortgage) {
        ...
        data.forEach(item => {
            ...
            // Форматируем число в зависимости от типа показателя
            let displayValue;
            if (isPremiumMortgage) {
                // Для льготной ипотеки - всегда 2 знака после запятой
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: function(context) {
                                ...
                                let displayValue;
                                if (isPremiumMortgage) {
                                    // Для льготной ипотеки - 2 знака после запятой
