    ...
    function showDetailView(key, category, subcategory) {
        ...
        // Активируем кнопку "Весь период"
        document.querySelectorAll('.quick-filter').forEach(btn => {
            btn.classList.remove('active');
            if (btn.getAttribute('data-months') === "0") {
    ...
    // Обработчики для кнопок быстрого выбора периода
    document.querySelectorAll('.quick-filter').forEach(btn => {
        btn.addEventListener('click', function() {
    ...
    // Функция для установки диапазона дат в зависимости от количества месяцев
    function setDateRange(months) {
        ...
        if (months > 0) {
            ...
            // Убедимся, что startDate не раньше minDate
        ...
        endDateInput.value = formatDateForInput(new Date(maxDate));

        // Обновляем представление
        updateDetailView();
    }

    // Функция для форматирования даты в формат, подходящий для input[type=date] (YYYY-MM-DD)
