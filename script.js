    ...
    applyFiltersBtn.addEventListener('click', updateDetailView);
    resetFiltersBtn.addEventListener('click', resetDateFilters);
    ...
    function updateTable(data) {
        ...
        data.forEach(item => {
            ...
            // Форматируем дату для льготной ипотеки с 2025 года
            if (isPremiumMortgage && item.date >= '2025-01-01') {
    ...
    function updateChart(data) {
        ...
        // ИЗМЕНЕНИЕ: Управление отступами таблицы
        positionTable();
    }

    // ИЗМЕНЕНИЕ: Упрощенная функция позиционирования таблицы
    function positionTable() {
        // Сброс отступа
        detailTableContainer.style.marginTop = '0';
        
        // Если есть второй график - добавляем отступ
        if (!premiumChartContainer.classList.contains('hidden')) {
            detailTableContainer.style.marginTop = '20px';
        }
    }
