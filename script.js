    ...
    function renderStandardChart(data, isPremiumMortgage) {
        ...
        const labels = data.map(item => {
            return isQuarterlyData ? getQuarterFromDate(item.date) : formatDate(item.date);
        });
    ...
    function renderPremiumMortgageChart(data) {
        ...
        if (pre2025Data.length > 0) {
            // Для квартальных данных используем кварталы
            const labels = pre2025Data.map(item => {
                return isQuarterlyData ? getQuarterFromDate(item.date) : formatDate(item.date);
            });
        ...
        if (post2025Data.length > 0) {
            ...
            const labels = post2025Data.map(item => {
                return isQuarterlyData ? getQuarterFromDate(item.date) : formatDateAsPeriod(item.date);
            });
