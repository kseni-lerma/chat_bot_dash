    ...
    function createTable(data, container, sectionTitle, formatAsPeriod, isPremiumMortgage) {
        ...
        data.forEach(item => {
            ...
            // Специальная обработка для льготной ипотеки - объем выданных кредитов
            const isSpecialPremium = isPremiumMortgage && 
                                  detailTitle.textContent.includes('Льготная ипотека') && 
                                  detailTitle.textContent.includes('Объем выданных кредитов');
            
            if (isSpecialPremium) {
                // Для льготной ипотеки - объем выданных кредитов: 2 знака после запятой
                displayValue = parseFloat(item.value).toLocaleString('ru-RU', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                });
            } else if (isPremiumMortgage) {
                // Для других льготных ипотечных показателей - 2 знака
                displayValue = parseFloat(item.value).toLocaleString('ru-RU', {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2
                });
            } else if (item.unit === 'ед.' || item.unit === 'млн ед.') {
                // Для показателей в единицах - целые числа
                displayValue = parseInt(item.value).toLocaleString('ru-RU');
            } else {
                // Для остальных - 2 знака после запятой
    ...
    function renderStandardChart(data, isPremiumMortgage) {
        ...
        // Проверяем специальный показатель
        const isSpecialPremium = isPremiumMortgage && 
                              detailTitle.textContent.includes('Льготная ипотека') && 
                              detailTitle.textContent.includes('Объем выданных кредитов');
                        ...
                        callbacks: {
                            label: function(context) {
                                ...
                                if (isSpecialPremium) {
                                    ...
                                    // Для других льготных показателей
                                    displayValue = value.toLocaleString('ru-RU', {
                                        minimumFractionDigits: 2,
                                        maximumFractionDigits: 2
                                    });
    ...
    function renderPremiumMortgageChart(data) {
        ...
        // Проверяем специальный показатель
        const isSpecialPremium = detailTitle.textContent.includes('Льготная ипотека') && 
                              detailTitle.textContent.includes('Объем выданных кредитов');
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            ...
                            callbacks: {
                                label: function(context) {
                                    let value = context.parsed.y;
                                    // Форматирование для специального показателя
                                    let displayValue;
                                    if (isSpecialPremium) {
                                        displayValue = value.toLocaleString('ru-RU', {
                                            minimumFractionDigits: 2,
