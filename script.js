    ...
    function updateTable(data) {
        ...
        // Добавляем разделитель для льготной ипотеки
        if (isPremiumMortgage) {
            ...
            // Секция данных до 2025 года
            if (pre2025Data.length > 0) {
                const sectionHeader = document.createElement('tr');
                sectionHeader.classList.add('section-header');
                sectionHeader.innerHTML = `<td colspan="2">Данные до 2025 года</td>`;
                detailTableBody.appendChild(sectionHeader);

                pre2025Data.forEach(item => {
                    appendTableRow(item, false);
                });
            }

            // Секция данных с 2025 года
            if (post2025Data.length > 0) {
                const sectionHeader = document.createElement('tr');
                sectionHeader.classList.add('section-header');
                sectionHeader.innerHTML = `<td colspan="2">Данные с 2025 года</td>`;
                detailTableBody.appendChild(sectionHeader);

                post2025Data.forEach(item => {
                    appendTableRow(item, true); // true = форматировать как период
                });
            }
        } else {
            // Обычное отображение для других показателей
            data.forEach(item => {
                appendTableRow(item, false);
            });
        }
    ...
    function appendTableRow(item, formatAsPeriod) {
